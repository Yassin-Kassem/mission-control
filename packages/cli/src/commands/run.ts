import { createMissionId, MissionRunner, MissionLearner, type DroneExecutor, type DroneManifest, ScoutExecutor, TesterExecutor, SecurityExecutor } from '@missionctl/core';
...
export async function runMission(description: string, projectDir: string, options: RunOptions = {}): Promise<void> {
  ...
  const learner = new MissionLearner(ctx.memory, ctx.signalStore, ctx.missionStore);
  learner.learnFromMission(missionId);
